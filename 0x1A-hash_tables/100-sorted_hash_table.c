...
int shash_table_set(shash_table_t *ht, const char *key, const char *value)
{
	...
	printf("   getting prevnode\n");
	prev_node = get_prev_node(ht->shead, key);
	printf("prev_node: %s | prev_node->snext: %s",
			prev_node->key, prev_node->snext->key);
	...
	{
		if (prev_node->snext)
			printf("prev_node->snext: %s\n", prev_node->snext->key);
		else
			printf("prev_node->snext is null\n"); 
...
shash_node_t *get_prev_node(shash_node_t *shead, const char *key)
{
	...
	while (temp)
	{
		...
		printf("comp: %s && %s [ret = ", key, temp->key);
					...
					const char *value)
{
	...
	printf("bucket inserting %s\n", key);
	exists = _key_exists(ht, index, key);
	if (exists)
	{
		printf("bucket insert: key exists section\n");
		free(exists->value);
